<p>IFRT (the Interim Framework Runtime) is a high-level ML runtime API that is
...
implementations.</p>
<p>The status quo is that PjRt ("pretty much just another runtime"), which is a
...
the APIs deviate, such migration may become harder.</p>
<p>IFRT requires careful prototyping as it is a new portable layer that would
...
XLA and will ensure coherent development of XLA and IFRT.</p>
<p>Once early IFRT prototyping is complete we will consult with stakeholders to
ensure both that all requirements are met, and also that there is a simple
migration path from PjRt to IFRT for teams who will benefit from migration.</p>